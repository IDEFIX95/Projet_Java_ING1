...
public class PoubelleIntelligente {
    ...
    public void attribuerPointsFidelity(Menage utilisateur, double quantity) {
        int points = (int) quantity * 10; // Exemple de conversion
        utilisateur.ajouterDepot(new HistoriqueDepot(0, utilisateur.getPointsFidelity(), this.idPoubelle, "", quantity, points));
    }

    public void envoyerNotification(Menage utilisateur) {
        System.out.println("Notification envoyée à " + utilisateur.getEmail() + ": Vous avez gagné " + utilisateur.getPointsFidelity() + " points de fidélité!");
    }
}
